...
The public browser build passes exact asset hashes, actual v1 save/preferences upgrade, and fresh cold-offline play. Windows v1.1.0 is published: GitHub's asset digests match the tested ZIP and checksum file, and both public download routes work. The source tag preserves the tested application inputs. The open performance follow-up remains visible, alongside the distinction between local, emulated, native and public-origin evidence.
