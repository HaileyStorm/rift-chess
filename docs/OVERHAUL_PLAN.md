...
Status: implementation and final acceptance active. The final 216-view board matrix has scoped independent acceptance; remaining motion, application, performance, full-game, and distribution gates are tracked in OVERHAUL_PROGRESS.md. This plan is the completion contract for the requested persistent Goal.
...
- Owner refinement: a legal destination click or keyboard Enter commits a Shift just as it commits an ordinary move. Remove the separate off-board Confirm Shift button. For both action kinds, selecting the same source cancels and selecting another legal source reselects it. Keep source selection and hover/focus previews inert; only a required promotion choice delays the atomic action.
...
- Owner release decision, September 8: after reviewing the final measured frame spikes, the owner accepted those limits for v1.1.0 publication. The engineering targets remain unmet and tracked in issue #1; this acceptance does not reclassify measurements as passes or waive any other delivery checks.
