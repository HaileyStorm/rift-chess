...
function measure(mode:'flat'|'screen'|'affine',count:number):number {
...
for(const mode of ['flat','screen','affine'] as const)measure(mode,12);
const output:Record<string,number[]>={};
for(const count of [8,12])for(const mode of ['flat','screen','affine'] as const)
  output[`${count}/${mode}`]=[];
for(let repeat=0;repeat<repetitions;repeat++) {
  const order=repeat%2?(['affine','screen','flat'] as const):
    (['flat','screen','affine'] as const);
...
console.log(JSON.stringify({size,repetitions,
  ...
  scope:'Pure emitted-JS top-face construction only for 8/12 affine projected slabs; no side faces/blit/browser/native/GPU'}));
