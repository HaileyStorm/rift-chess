...
const worker = await Bun.build({ ...common, entrypoints: [path.join(root, 'bend2/web/worker.ts')], plugins: [plugin] });
...
const main = await Bun.build({ ...common, entrypoints: [path.join(root, 'bend2/web/main.ts')], define: { __BEND_WORKER__: JSON.stringify(`./${workerName}`) } });
...
const css = fs.readFileSync(path.join(root, 'bend2/web/style.css'));
const cssName = `style-${digest(css).slice(0, 12)}.css`;
fs.writeFileSync(path.join(out, cssName), css);
fs.writeFileSync(path.join(out, 'index.html'), fs.readFileSync(path.join(root, 'bend2/web/index.html'), 'utf8')
...
fs.writeFileSync(path.join(out, 'sw.js'), fs.readFileSync(path.join(root, 'bend2/web/sw.js'), 'utf8')
...
fs.writeFileSync(path.join(out, 'build.json'), JSON.stringify({ schema: 'rift-bend-browser/1', builtAt: new Date().toISOString(), version, sourceRevision, sourceDirty, draft, semanticSha256: semantic?.sha256 ?? null, pixelSemanticSha256: pixels?.sha256 ?? null, toolchain: JSON.parse(fs.readFileSync(path.join(root, 'bend2/TOOLCHAIN.json'))), files }, null, 2) + '\n');
