...
const macroSquare = n => (Number(n[1]) - 1) * 16 + (n.charCodeAt(0) - 65) * 2;
...
async function load(value) { await page.evaluate(v => window.rift.loadScenario(v), value); await page.locator('[data-camera="top"]').click(); if (await page.locator('#shift-mode').getAttribute('aria-pressed') === 'true') await page.locator('#shift-mode').click(); await page.waitForTimeout(100); }
...
async function act(type, from, to, promotion) {
  const before = await observe();
  if (type === 'shift') { if (await page.locator('#shift-mode').getAttribute('aria-pressed') !== 'true') await page.locator('#shift-mode').click(); await square(macroSquare(from)); await square(macroSquare(to)); }
  else { await square(index(from)); await square(index(to)); }
  if (promotion) { await page.locator('#promotion-dialog').waitFor({ state: 'visible' }); assert.equal((await observe()).revision, before.revision); await page.locator(`#promotion-dialog button[value="${promotion}"]`).click(); }
  await page.waitForFunction(r => window.rift.getObservation().revision === r + 1, before.revision); await page.waitForTimeout(550);
}
...
try {
  ...
  await test('all progress policies at 100 and prompt dismissal', async () => {
    ...
    for (const policy of ['prompt', 'auto100', 'off']) {
      ...
      if (policy === 'off') assert.equal(await page.locator('.counter').isVisible(), false);
    }
  });
  await test('threefold survives actual browser reload', async () => { await load(fixtures.find(f => f.name === 'opening_B').record); await act('shift', 'A2', 'B2'); await act('shift', 'B2', 'A2'); const saved = await record(); await page.reload({ waitUntil: 'domcontentloaded' }); await page.waitForFunction(() => Boolean(window.rift)); assert.deepEqual(await record(), saved); await page.locator('[data-camera="top"]').click(); await act('shift', 'A2', 'B2'); await act('shift', 'B2', 'A2'); assert.equal((await observe()).outcome.reason, 'threefold'); });
  ...
  await test('nonmoving hotseat player can resign; earlier replay stays ongoing', async () => { await load(fixtures.find(f => f.name === 'opening_B').record); await act('move', 'e2', 'e4'); await page.getByRole('combobox', { name: 'Match controls for' }).selectOption('1'); await page.locator('#resign').click(); assert.equal((await observe()).outcome.winner, -1); await page.locator('#replay').click(); await page.locator('#replay-back').click(); assert.equal(await page.locator('#check').innerText(), 'Position steady'); await page.locator('#replay-exit').click(); });
