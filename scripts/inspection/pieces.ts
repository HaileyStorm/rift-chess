...
type Settings = { family: PieceFamily; material: MaterialStyle; view: 'front' | 'side' | 'three-quarter' | 'top'; shadows: boolean; diagnostic: 'material' | 'neutral' | 'normal' | 'depth' | 'wireframe'; zoom: number; azimuth?: number | null; shadowBias?: number; normalBias?: number };
...
key.shadow.mapSize.set(1024, 1024); key.shadow.camera.left = -1.3; key.shadow.camera.right = 1.3; key.shadow.camera.top = 1.3; key.shadow.camera.bottom = -1.3; key.shadow.camera.near = .1; key.shadow.camera.far = 15; key.shadow.bias = .00025; key.shadow.normalBias = .003;
...
function render(settings: Partial<Settings> = {}) {
  state = { ...state, ...settings, azimuth: settings.azimuth ?? null }; renderer.shadowMap.enabled = state.shadows; key.castShadow = state.shadows; floor.receiveShadow = state.shadows; (floor.material as THREE.Material).needsUpdate = true; floor.visible = state.diagnostic === 'material' || state.diagnostic === 'neutral';
  key.shadow.bias = settings.shadowBias ?? .00025; key.shadow.normalBias = settings.normalBias ?? .003;
  ...
  if (state.azimuth !== undefined && state.azimuth !== null) { const angle = state.azimuth * Math.PI / 180; camera.position.set(Math.sin(angle) * 4, 1.9, Math.cos(angle) * 4); camera.lookAt(0, .66, 0); camera.updateMatrixWorld(); }
  ...
  if (state.azimuth !== undefined && state.azimuth !== null) document.querySelector('#caption')!.textContent += ` / orbit ${state.azimuth} degrees`;
  if (settings.shadowBias !== undefined) document.querySelector('#caption')!.textContent += ` / bias ${key.shadow.bias} / normal bias ${key.shadow.normalBias}`;
