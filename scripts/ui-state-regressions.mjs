...
try {
  ...
  await test('keyboard-only source, destination focus, and Enter commit keep hints hidden', async () => {
    ...
    await driver.openDrawer('Record'); await page.locator('#replay').click(); await page.locator('#replay-back').click(); await page.locator('#scene').focus(); await focusKeyboardSquare(12); assert.match(await page.locator('#selection').innerText(), /^e2: White pawn\. Replay position\.$/); await page.locator('#replay-exit').click();
  });
